...
//  */
// export function action(action: Action, interaction: Interaction): boolean {
//   const invalidations: string[] = []

//   // cooldown
//   if (action.cooldown) {
//     const timestamp = handleCooldown(action, interaction.user.id)

//     if (timestamp) {
//       // const hammertime = `<t:${Math.round(timestamp * 0.001)}:R>`
//       // invalidations.push(`Action is \`overheated\`. Cooldown ${hammertime}`)
//       logger.warn(`${interaction.user.tag} called <overheated> action <${action.data.name}>`)
//       return false
//     }
//   }

//   // devsOnly
//   if (action.devsOnly && !isDeveloper(interaction.user)) {
//     // invalidations.push(`Actions is \`devs-only\``)
//     logger.warn(`${interaction.user.tag} called <devs-only> action <${action.data.name}>`)
//     return false
//   }

//   // testOnly
//   if (action.testOnly && interaction.guild && !isTest(interaction.guild)) {
//     // invalidations.push(`Actions is \`test-server-only\``)
//     logger.warn(`${interaction.user.tag} called <test-server-only> action <${action.data.name}>`)
//     return false
//   }
...
//     logger.warn(`${interaction.user.tag} called <deleted> action <${action.data.name}>`)
//     return false
//   }

//   // if (invalidations) {
//   //   // send dm or reply ephemeral
//   //   return false
//   // }
